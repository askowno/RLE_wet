...
RLE <- RLE %>%
  select(type, hgm, RLE18, RLE24, A3sa_18, A3sa_24, type_m2) %>%
  rename(RLEsa18 = A3sa_18, RLEsa24 = A3sa_24, zone = hgm) %>%
  mutate(extent = type_m2/1e6)
EPL <- EPL %>%
...
```{r}
ETS <- read.csv("C:/Rdata/RLE_wet/outputs/rle_wet_metrics_per_type.csv") %>% select(type, hgm, A3sa_18, A3sa_24, type_m2) %>%  rename( ETS18 = A3sa_18, ETS24 = A3sa_24, zone = hgm) %>%
mutate(extent = type_m2/1e6) %>% select(-type_m2)
