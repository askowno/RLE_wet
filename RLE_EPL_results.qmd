...
RLE <- read.csv("outputs/rle_wet_metrics_per_type.csv")
EPL <- read.csv("https://raw.githubusercontent.com/askowno/EPL_wet/main/outputs/epl_wet_metrics_per_type.csv")
...
Spatial data output for maps

```{r}
# Prepare to output spatial data with combined RLE and EPL results

wet_results_vector <- wet %>%
  left_join(wet_results_for_integration, by = "type" )

st_write(
  wet_results_vector,
  "C:/Users/skownoa/Dropbox/NBAwork/Wetlands/wet_results_vector.gpkg", layer = "wetland_results_20250823",
  delete_dsn = TRUE
)

# make one I can open in ARCGIS - must move to unprojected (WKT conflict r to ARC)
st_write(
  st_transform(wet_results_vector, crs = 4326),
  "C:/Users/skownoa/Dropbox/NBAwork/Wetlands/wet_results_vector_dd.gpkg", layer = "wetland_results_20250823",
  delete_dsn = TRUE
)
```
