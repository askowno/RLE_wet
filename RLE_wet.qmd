...
# Read PAs (projected layer) not required but makes correcting projection easy
...
# check cols
# sort(unique(wetlands$type)) 
# sort(unique(wetlands$bioregion)) 
# sort(unique(wetlands$hgm1)) 
# sort(unique(wetlands$EC18VEG)) 
```
...
#write.csv(wet_tb, "outputs/wet_tb__nba25_inputdata.csv")
  
# make a simple table of TYPE and HGM to join with results later 
  type_hgm_tb <- wet_tb %>% select(type, hgm1) %>% distinct(type, .keep_all = TRUE) %>% rename(hgm = hgm1)

#   # check cols
# sort(unique(wet_tb$type)) 
# check <- wet_tb %>%
#   group_by(type) %>%
#   summarise(tot_ha = sum(area_m2/10000) )
```
...
write.csv(eoo_df, "outputs/EOO_all.csv")
 eoo_df <- read.csv("outputs/EOO_all.csv")
...
```{r}
## STOP - dont run this unless you what all the spatial intersections between wetlands and land cover recomputed . Skip to line 140 and import the metrics required to proceed. 

# cross tab wetlands with land cover 1990, 2014, 2018 and 2022 and calculate extent lost and rate of loss
...
########################START HERE to avoid rerunning spatial step
...
wet_type_24 <- wet_tb %>%
  ...
  ungroup() %>%
  pivot_wider(names_from = EC22ALL, values_from = area24_sum, values_fill = list(area24_sum = 0)) %>%
...
wet_type_18 <- wet_tb %>%
  ...
  ungroup() %>%
  pivot_wider(names_from = EC18ALL, values_from = area18_sum, values_fill = list(area18_sum = 0)) %>%
...
**Prepare for focused assessments of each element of decline**

Using PESALL PESVEG PESWQ PESGEO PESHYD separately in a D2 assessment (this prevents the overall Wet Health score dominating the RLE) and allows us to see the symptom of decline more clearly

```{r}
# Calculate D2 based on rate of decline in PES score over period 2018-2024 projected forward to 2068, then assess % of decline over 50years - This would be much worse if we had real PES trends 
D2fin <- wet_tb %>%
    group_by(type) %>%
    summarise(across(
    c(call24 = PES22ALL,
 ...
 mutate(call68 = pmin(pmax(call18 + ((call24 - call18) / (2022 - 2018) * 46), 0), 1)) %>%
  mutate(cond_decline = pmin(pmax((call18 - call68) / call18, 0), 1)) %>% 
...
#D2 decline in PES  over 50 year period 
    mutate(D2_24 = case_when(    cond_decline >= 0.8 ~ "CR", 
                                 ...
                                 cond_decline >= 0.3 ~ "VU", 
                                 TRUE ~ "LC")) %>%
  mutate(D2_24 = factor(D2_24, levels = c("LC", "VU", "EN", "CR"), ordered = TRUE)) %>%
  mutate(D2_18 = D2_24) %>%
  select(type, call18,call24,call68,D2_18, D2_24)
```

Alternative methods (full methods) for D3

```{r}
# Calculate D3 for each element of PES individually and overall
# In the launch version of this analysis (v1.0.0)  90% disruption of biotic process was now assumed for PES condition E and F. In this revised  version v1.2.0 PES D, E and F are considered 90% biotic disruption.  

# Make a function to calculate D3 using extent in CDEF, DEF, and EF condition
D3calc <- function(data, ec_col, out_col) {
  data %>%
    group_by(type, .data[[ec_col]]) %>%
    summarise(area24_sum = sum(as.numeric(area_m2), na.rm = TRUE), .groups = "drop") %>%
    pivot_wider(
      names_from = all_of(ec_col),
      values_from = area24_sum,
      values_fill = list(area24_sum = 0)
    ) %>%
    mutate(type24_m2   = rowSums(across(c(A, B, C, D, E, F))),
           prp24_bcdef = (B + C + D + E + F)/type24_m2,
           prp24_cdef  = (C + D + E + F) / type24_m2,
           prp24_def   = (D + E + F) / type24_m2,
           prp24_ef    = (E + F) / type24_m2,
           "{out_col}" := case_when(
             prp24_def  >= 0.9                        ~ "CR",
             prp24_def  >= 0.7 | prp24_cdef >= 0.9     ~ "EN",
             prp24_def  >= 0.5 | prp24_cdef >= 0.7     ~ "VU",
             TRUE                                    ~ "LC"
           )) %>%
    select(type, all_of(out_col))
}

# apply function to EC data 
library(purrr)

D3fin <- list(
  D3calc(wet_tb, "EC22ALL", "D3_24"),
  D3calc(wet_tb, "EC22VEG", "D3veg_24"),
  D3calc(wet_tb, "EC22WQ",  "D3wq_24"),
  D3calc(wet_tb, "EC22GEO", "D3geo_24"),
  D3calc(wet_tb, "EC22HYD", "D3hyd_24"),
  D3calc(wet_tb, "EC18ALL", "D3_18"),
  D3calc(wet_tb, "EC18VEG", "D3veg_18"),
  D3calc(wet_tb, "EC18WQ",  "D3wq_18"),
  D3calc(wet_tb, "EC18GEO", "D3geo_18"),
  D3calc(wet_tb, "EC18HYD", "D3hyd_18")
) %>%
  reduce(left_join, by = "type") %>%
 # make all assessments factors to get MAX 
  mutate(across( c(D3_18, D3veg_18, D3wq_18, D3geo_18, D3hyd_18, D3_24, D3veg_24, D3wq_24, D3geo_24, D3hyd_24), 
                   ~ factor(., levels = c("LC", "VU", "EN", "CR"), ordered = TRUE)  ))  %>%
 # Ensure highest risk category is assigned 
   rowwise() %>%
     mutate(D3fin_18 = max(c_across(c(D3_18, D3veg_18, D3wq_18, D3geo_18, D3hyd_18)), na.rm = TRUE), 
            D3fin_24 = max(c_across(c(D3_24, D3veg_24, D3wq_24, D3geo_24, D3hyd_24)), na.rm = TRUE))
...
**Build table of Assessments B1aii and bring in A2b A3, then add ETS (A3sa) , and join with D2 and D3 from above**
...
wet_type_metrics <- wet_type_24 %>%
  ...
  left_join(type_lc_metrics) %>%
  mutate(prp24_def = (D+E+F)/type24_m2, prp18_def = (D18+E18+F18)/type24_m2) %>%
...
# Add van Deventer 2018 ETS method: assuming A and B are intact rest is lost but also counting C towards VU (use DDfec as PES C)
    ...
    mutate(A3sa_18 = case_when( prp18_ab < 0.2 ~ "CR", 
                                ...
                                prp18_abc < 0.6 ~ "VU", 
                                 TRUE ~ "LC")) %>% 
# Bring in D2 and D3 assessments
  left_join(D2fin, by = "type") %>%
  left_join(D3fin, by = "type") %>%
# re order cols 
    relocate(A2b_18, A3_18,  A3sa_18, B1ai_18, B1aii_18, D2_18, D3_18, D3veg_18, D3wq_18, D3geo_18, D3hyd_18, D3fin_18, A2b_24, A3_24, A3sa_24, B1ai_24, B1aii_24, D2_24, D3_24, D3veg_24, D3wq_24, D3geo_24, D3hyd_24, D3fin_24,   .after = call68) %>%
  # overall RLE outcome
  # make all assessments factors to get MAX 
    mutate(across( c(A2b_18, A3_18,  A3sa_18, B1ai_18, B1aii_18, D2_18, D3_18, D3veg_18, D3wq_18, D3geo_18, D3hyd_18, D3fin_18, A2b_24, A3_24, A3sa_24, B1ai_24, B1aii_24, D2_24, D3_24, D3veg_24, D3wq_24, D3geo_24, D3hyd_24, D3fin_24), 
                   ~ factor(., levels = c("LC", "VU", "EN", "CR"), ordered = TRUE)  ))  %>%
 # Ensure highest risk cateroy is assiged 
   rowwise() %>%
     mutate(RLE18 = max(c_across(c(A2b_18, A3_18,  B1ai_18, B1aii_18, D2_18, D3fin_18)), na.rm = TRUE), 
            RLE24 = max(c_across(c(A2b_24, A3_24,  B1ai_24, B1aii_24, D2_24, D3fin_24)), na.rm = TRUE),
            ETS18 = A3sa_18, ETS24 = A3sa_24) %>%
   ungroup() %>%
      # add in HGM type
...
# make summary of extent of RLE 2024 for ploting 
 rle24_ext <- wet_type_metrics %>%
  mutate(RLE24 = factor(RLE24, levels = expected_levels)) %>%   
  group_by(RLE24, hgm) %>%
  summarise(extent = round(sum(type24_m2, na.rm = TRUE) / 1000000, 0), .groups = 'drop') %>%
  complete(RLE24, hgm, fill = list(extent = 0)) %>%  # <-- fill correct col
  pivot_wider(
    names_from = RLE24, 
    values_from = extent, 
    values_fill = 0
  ...
  # Add TOTAL row (column-wise sums)
  bind_rows(
    summarise(., hgm = "Total", across(where(is.numeric), ~ sum(.x, na.rm = TRUE)))
  ) %>%
  mutate(
    hgm = factor(hgm, 
                 levels = c("Total", "Unchannelled valley-bottom", "Seep", "Floodplain", "Depression"), 
                 ordered = TRUE)
  ) %>%
   arrange(hgm)
...
# 2018 summaries IUCN RLE approach 
 ...
 rle18_ext <- wet_type_metrics %>%
  ...
  complete(RLE18, hgm, fill = list(extent = 0)) %>%  # <-- fill correct col
   pivot_wider(names_from = RLE18, 
...
rle24sa_sum <- wet_type_metrics %>%
    group_by(ETS24, hgm) %>%
  summarise(count = n(),.groups = 'drop') %>%
  complete(ETS24, hgm, fill = list(count = 0)) %>%  # <--- fills missing combos
...
# COUNTS - summary for 2018 using SA methods 
   rle18sa_sum <- wet_type_metrics %>%
    group_by(ETS18, hgm) %>%
  summarise(count = n(),.groups = 'drop') %>%
  complete(ETS18, hgm, fill = list(count = 0)) %>%  # <--- fills missing combos
...
#EXENTS - summary for 2024  using SA methods 
 rle24sa_ext <- wet_type_metrics %>%
  ...
  complete(ETS24, hgm, fill = list(extent = 0)) %>%  # <-- fill correct col
   pivot_wider(names_from = ETS24, 
              values_from = extent, values_fill = 0) %>%
    relocate(VU, .after = EN) %>%
   # Add row-wise TOTAL
 ...
 rle18sa_ext <- wet_type_metrics %>%
    group_by(ETS18, hgm) %>%
  summarise(extent = round(sum(type18_m2)/1000000, 0),.groups = 'drop') %>%
  complete(ETS18, hgm, fill = list(extent = 0)) %>%  # <-- fill correct col
   pivot_wider(names_from = ETS18, 
              values_from = extent, values_fill = 0) %>%
    relocate(VU, .after = EN) %>%
   # Add row-wise TOTAL
...
## Make plots of extent per hgm zone RLE approach  #########################################################
...
ggsave(
  filename = "outputs/rle18_wet_barplot_count.jpeg", # File name
