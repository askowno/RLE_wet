...
wet <- wetlands %>%
  ...
  mutate(cond24 = ifelse(is.na(pes24) & cond24 == 0, NA, cond24)) %>% 
  # now deal with some pes but missing cond (D2)
    mutate(cond24 = case_when((cond24 == 0 | is.na(cond24)) & pes24 == "A" ~ 95, 
                            (cond24 == 0 | is.na(cond24)) & pes24 == "B" ~ 65,
                            (cond24 == 0 | is.na(cond24)) & pes24 == "C" ~ 40,
                            (cond24 == 0 | is.na(cond24)) & pes24 == "D" ~ 20,
                            (cond24 == 0 | is.na(cond24)) & pes24 == "E" ~ 8,
                            (cond24 == 0 | is.na(cond24)) & pes24 == "F" ~ 4, 
                            TRUE ~ cond24)) %>%
    mutate(cond18 = case_when((cond18 == 0 | is.na(cond18)) & pes18 == "A" ~ 95, 
                              (cond18 == 0 | is.na(cond18)) & pes18 == "B" ~ 65,
                              (cond18 == 0 | is.na(cond18)) & pes18 == "C" ~ 40,
                              (cond18 == 0 | is.na(cond18)) & pes18 == "D" ~ 20,
                              (cond18 == 0 | is.na(cond18)) & pes18 == "E" ~ 8,
                              (cond18 == 0 | is.na(cond18)) & pes18 == "F" ~ 4, 
                              TRUE ~ cond18)) %>%
...
# average condition estimates for use in D2 rate of change  - this is an adjusted per type area reflecting condition of the type
wet_scores <- wet_tb %>%
    group_by(type) %>%
    summarise(score24 = sum(cond24/100 * area_m2, na.rm = TRUE), 
              score18 = sum(cond18/100 * area_m2, na.rm = TRUE)) %>% # scale area of feature using condition
  ungroup() %>%
  left_join(wet_type_24, by  = "type") %>% 
  mutate(condindex24 = pmin(pmax(score24 / type24_m2, 0), 1), # divide by area of type to make an index
  condindex18 = pmin(pmax(score18 / type24_m2, 0), 1) ) %>%
  select(type, condindex24, condindex18)
...
# Metrics for RLE  
  ...
  # Calculate proportion in each pes group of severity   
...
#D2 decline in PES  over 50 year period 
    mutate(D2_24 = case_when(    cond_decline >= 0.8 ~ "CR", 
                                 ...
                                 cond_decline >= 0.3 ~ "VU", 
                                 TRUE ~ "LC")) %>%
   mutate(D2_18 = D2_24) %>% # PES 2014 unreliable to cannot be used to get rate - assumed to be same t1 and t2 
...
#  B1aii using ongoing decline calculated above add 2018 when PES data is available
   ...
   mutate(B1aii_18 = case_when( pes_decline1824 == "decline1824" & eoo_km2  <= 2000 ~ "CR",
                                ...
                                pes_decline1824 == "decline1824" & eoo_km2  <= 50000 ~ "VU",
                                 TRUE ~ "LC")) %>%
   # re order cols 
    relocate(B1aii_18, D2_18,D3_18, D3b_18, B1aii_24, D2_24, D3_24, D3b_24,  .after = pes_decline1824) %>%
  # overall RLE outcome
  # make all assessments factors to get MAX 
    mutate(across( c(B1aii_18, D2_18,D3_18, D3b_18, B1aii_24, D2_24, D3_24, D3b_24), 
                   ~ factor(., levels = c("LC", "VU", "EN", "CR"), ordered = TRUE)  ))  %>%
 # Ensure highest risk cateroy is assiged 
   rowwise() %>%
     mutate(RLE18 = max(c_across(c(B1aii_18, D2_18,D3_18, D3b_18)), na.rm = TRUE), 
            RLE24 = max(c_across(c(B1aii_24, D2_24, D3_24, D3b_24)), na.rm = TRUE)) %>%
   ungroup() %>%
