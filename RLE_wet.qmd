...
wet <- wetlands %>%
  ...
  mutate(EC22ALL = ifelse(is.na(NBAPES2025)|NBAPES2025 == "not assessed", "DD", NBAPES2025)) %>%
  mutate(EC18ALL = ifelse(is.na(EC18ALL), EC22ALL, EC18ALL)) %>% # fill NAs for PES2018 with 2022 values
  select(-NBAPES2025) %>%
...
wet_v <- wet %>%
   rename(pes18 = EC18ALL, pes25 = EC22ALL ) %>%
   left_join(st_drop_geometry(wetlands), by = "fid") %>%
   relocate(pes25, .after = pes18) %>%
   mutate(as.numeric(area_m2))

wt_v_dd <- st_transform(wet_v, 4326)
st_write(wt_v_dd, "C:/Users/skownoa/Dropbox/NBAwork/Wetlands/wetlands_20250730.gpkg", delete_dsn = TRUE) 
...
type_lc_metrics <- type_ras_ext %>%
  ...
  mutate(B1ai_24 = case_when( rod9022 >= 0.4  & eoo_km2  <= 2000 ~ "CR",
                              rod9022 >= 0.4  & eoo_km2   <= 20000 ~ "EN",
                             rod9022 >= 0.4  & eoo_km2   <= 50000 ~ "VU",
                                 TRUE ~ "LC")) %>%
...
# assessments 2018
  ...
  mutate(B1ai_18 = case_when( rod9018 >= 0.4  & eoo_km2  <= 2000 ~ "CR",
                              rod9018 >= 0.4  & eoo_km2   <= 20000 ~ "EN",
                             rod9018 >= 0.4  & eoo_km2   <= 50000 ~ "VU",
                                 TRUE ~ "LC")) %>%
...
# add B1i using ongoing decline calculated above add 2018 when PES data is available
   mutate(B1aii_24 = case_when( pes_decline1824 == "decline1824" & eoo_km2  <= 2000 ~ "CR",
                              ...
                              pes_decline1824 == "decline1824" & eoo_km2  <= 50000 ~ "VU",
                                 TRUE ~ "LC")) %>%
            mutate(B1aii_24 = factor(D3_24, levels = c("LC","VU", "EN", "CR"), ordered = TRUE)) %>%
   mutate(B1aii_18 = case_when( pes_decline1824 == "decline1824" & eoo_km2  <= 2000 ~ "CR",
                              pes_decline1824 == "decline1824" & eoo_km2  <= 20000 ~ "EN",
                              pes_decline1824 == "decline1824" & eoo_km2  <= 50000 ~ "VU",
                                 TRUE ~ "LC")) %>%
            mutate(B1aii_18 = factor(D3_18, levels = c("LC","VU", "EN", "CR"), ordered = TRUE)) %>%
   # re order cols 
  ...
  # overall RLE outcome
   ...
   # MANUAL ADJUSTMENT OF Albany Thicket_Depression RLE24 to CR from EN to match RLE18 - makes plots work as no CRs 
  mutate(RLE24 = case_when( type == "Albany Thicket_Floodplain" ~ "CR",
      TRUE ~ as.character(RLE24)), RLE24 = factor(RLE24, levels = c("LC","VU", "EN", "CR"), ordered = TRUE))
